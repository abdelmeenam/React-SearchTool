       ...
       overflow-hidden border border-white/20 dark:border-gray-700/50">
        ...
        <div className="px-5 py-3 bg-gradient-to-r from-blue-600 to-blue-700 flex justify-between items-center relative">
          ...
          <div className="min-w-0 relative">
            <h3 className="text-lg font-bold text-white truncate pr-6">{drug.name}</h3>
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { label: 'Acquisition', value: formatCurrency(drug.acq), color: 'blue' },
              { label: 'Insurance', value: formatCurrency(drug.awp), color: 'purple' },
              { label: 'Status', value: 'In Stock', color: 'green' },
              { label: 'Copay', value: formatCurrency(drug.acq ?? 0), color: 'amber' }
            ].map((item, index) => (
              <div 
                key={index}
              ...
              >
                <p className={`text-xs text-${item.color}-600 dark:text-${item.color}-300`}>{item.label}</p>
                <p className="text-base font-semibold mt-1 dark:text-white">{item.value}</p>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-3 text-sm">
            <div className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-gray-700">
              ...
              <span className="font-medium text-gray-800 dark:text-gray-200">{drug.ndc}</span>
            </div>
            <div className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-gray-700">
              <span className="text-gray-500 dark:text-gray-400 flex items-center">
                <svg className="w-3.5 h-3.5 mr-2 opacity-70" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <span className="font-medium text-gray-800 dark:text-gray-200 truncate max-w-[160px]">{drug.acq}</span>
            </div>
            <div className="flex justify-between items-center py-2">
              ...
              <span className="font-medium text-gray-800 dark:text-gray-200">{formatCurrency(drug.acq)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="px-5 py-3 bg-gray-50/80 dark:bg-gray-700/30 border-t border-gray-200/50 dark:border-gray-700/50 flex justify-end space-x-3">
          ...
          {cartItems.some((item) => item.id === drug.ndc) ? (
            ...
            <button
              ...
              className="px-4 py-2 text-sm font-medium text-white bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg shadow-sm hover:shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 dark:focus:ring-offset-gray-700 transition-all duration-150 flex items-center"
            >
